    ...
    UI.component('bixshopform', {
        ...
        init: function () {
            ...
            this.find('div.productItem').each(function () {
                $this.$producten[$(this).attr('id')] = $(this);
            });
...
var bixZooShopClass = new Class({
    ...
    loadDom: function (formID, options) {
        ...
        this.geo = new bixGeo('userdata', {
            debug: true, onComplete: function (result) {
    ...
    calculate: function () {
        ...
        Object.each(this.products, function (prodData, productID) {
            ...
            Object.each(prodData.data.optionData, function (optieInfo, optienaam) {
                Object.each(optieInfo, function (selectInfo, optievalue) {
                    if (typeof prodFormData[selectInfo.type] == 'undefined') {
                        prodFormData[selectInfo.type] = {};
                    }
                    if (typeof prodFormData[selectInfo.type][optienaam] == 'undefined') {
                        prodFormData[selectInfo.type][optienaam] = {};
                    }
        ...
        if (price > 0) {
            if (price > 10) { //order meer dan 10
                ...
                if (pcode != '' && pcode.match(/^[0-9]{4}[A-Z]{2}/)) { //adres niet valide
                    ...
                    var UT = ['7522NB', '7522ND', '7522NH', '7522LW', '7522EA', '7521NJ', '7522LV', '7522NJ', '7522LP', '7522LV', '7522NM', '7522NL', '7522MJ', '7522MG', '7522NR', '7522NE', '7522MJ', '7522NC', '7522NR', '7521AN', '7521PA', '7521AG', '7547AN', '7511GB', '7521AG', '7522NR', '7514AE', '7513EA', '7522NH', '7522ND', '7522PB', '7522NR', '7522ND', '7521PT'];
        ...
        if (orderValid) { //kosten toevoegen
            ...
            if (bezorgKosten > 0) {
                validMessage += ' Kosten: € ' + bezorgKosten + ',-.'
            }
...
var bixGeo = new Class({
    ...
    initialize: function (control, options) {
        ...
        try {
            ...
            $$('[name^=' + control + ']').each(function (inputEl) {
                if (inputEl.hasClass('postcode')) {
                    self.adresFields.postcodeEl = inputEl;
                }
                if (inputEl.hasClass('huisnummer')) {
                    self.adresFields.huisnummerEl = inputEl;
                }
                if (inputEl.hasClass('huisnummer_toevoeging')) {
                    self.adresFields.huisnummertvEl = inputEl;
                }
                if (inputEl.hasClass('straat')) {
                    self.adresFields.straatEl = inputEl;
                }
                if (inputEl.hasClass('plaats')) {
                    self.adresFields.plaatsEl = inputEl;
                }
                if (inputEl.hasClass('land')) {
                    self.adresFields.landEl = inputEl;
                }
                if (inputEl.hasClass('type')) {
                    self.typeField = inputEl;
                }
                if (inputEl.hasClass('lat')) {
                    self.latField = inputEl;
                }
                if (inputEl.hasClass('lon')) {
                    self.lonField = inputEl;
                }
    ...
    lookup: function () {
        ...
        if (this.adresFields.landEl.get('value') != 'NL' || !postcode || !huisnummer) {
            return;
        }
