...
*/
class ElementShopoptie extends ElementOption {
	...
	public function render($params = array()) {
		...
		foreach ($this->config->get('option', array()) as $option) {
			if (in_array($option['value'], $selected_options)) {
				...
				$html[]  = '<div class="grid-block"><input data-zoo-product-option="'.$option['value'].'"
						id="shopoptie-'.$this->_item->id.''.$option['value'].$i.'" type="checkbox"
						name="shopdata[product'.$this->_item->id.']['.$option['value'].']"
						value="1" /><label
						for="shopoptie-'.$this->_item->id.''.$option['value'].$i++.'"
